...
var OmniSharpAtom = (function () {
    ...
    OmniSharpAtom.prototype.activate = function (state) {
        ...
        if (dependencyChecker.findAllDeps(this.getPackageDir())) {
            ...
            var started = rx_1.Observable.merge(this.getFeatures(featureList, whiteList, "atom"), this.getFeatures(featureList, whiteList, "features")).filter(function (l) {
                ...
                .flatMap(function (z) { return z.load(); })
                .toArray()
                .share();
            ...
            this.disposable.add(started.subscribe(function (features) {
                ...
                _.each(features, function (f) {
                    ...
                    if (whiteListUndefined && _.has(_this.config, key)) {
                        _this.disposable.add(atom.config.observe("omnisharp-atom." + key, function (enabled) {
                            if (!enabled) {
                                try {
                                    value.dispose();
                                }
